...
impl <T> Timer<T, Uninit> {
    ...
    pub(crate) fn register(self, ring: &mut io_uring::IoUring, domain_id: u16, kind: user_data::Timer) -> std::io::Result<Timer<T, Init>> {
        let Self {
            state: Uninit {
                state,
                flags,
        ...
        unsafe {
            ring.submission().push(&poll).expect("could not push sqe entry");
        }
...
impl <T> Timer<T, Init> {
    ...
    pub(crate) fn try_update_duration(&mut self, duration: Duration, flags: Option<SetTimeFlags>) {
        match self.state.prev_state {
            TimerState::Periodic {
                ...
                let flags = flags.unwrap_or(SetTimeFlags::Default);
                self.state.prev_state = self.state.tfd.set_state(TimerState::Periodic {
                    interval: duration,
                    current: duration,
                }, flags);
            }
    ...
    pub(crate) fn new_immediate(t: T, state: TimerState, flags: Option<SetTimeFlags>, ring: &mut io_uring::IoUring, domain_id: u16, kind: user_data::Timer) -> std::io::Result<Self> {
    ...
    pub(crate) fn new_immediate_periodic(t: T, duration: Duration, flags: Option<SetTimeFlags>, ring: &mut io_uring::IoUring, domain_id: u16, kind: user_data::Timer) -> std::io::Result<Self> {
        Self::new_immediate(t, TimerState::Periodic{current: duration, interval: duration}, flags, ring, domain_id, kind)
    }

    pub(crate) fn new_immediate_oneshot(t: T, duration: Duration, flags: Option<SetTimeFlags>, ring: &mut io_uring::IoUring, domain_id: u16, kind: user_data::Timer) -> std::io::Result<Self> {
        Self::new_immediate(t, TimerState::Oneshot(duration), flags, ring, domain_id, kind)
    }
}

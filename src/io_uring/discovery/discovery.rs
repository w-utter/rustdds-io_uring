...
impl <D: Keyed> Cache<D, timer_state::Uninit> {
    fn new(topic_name: &str, topic_type: &str, qos: QosPolicies, writer_guid: GUID, reader_guid: GUID) -> Self {
        ...
        let writer_heartbeat_period = qos_3
          ...
          .map(|hbp| {
            ...
            if let Some(Liveliness::ManualByTopic { lease_duration }) =
              qos_3.liveliness
            {
    ...
    fn register(self, ring: &mut IoUring, domain_id: u16) -> std::io::Result<Cache<D, timer_state::Init>> {
        let Self {
            topic,
            read,
            datasample,
...
impl <D: Keyed> Cache<D, timer_state::Init> {
    ...
    fn write<SA: SerializerAdapter<D>>(&mut self, data: D, udp_sender: &UDPSender, ring: &mut IoUring) -> WriteResult<(), D> {
        // serialize
        ...
        if let Some(cc) = self.history_buffer.get_change(timestamp) {
            ...
            for msg in messages_to_send {
                ...
                crate::io_uring::rtps::Writer::send_message_to_readers(Endianness::LittleEndian, DeliveryMode::Multicast, msg, &mut self.reader_proxies.values(), udp_sender, ring);
            }
        }
        Ok(())
    }
    ...
    fn send_preemptive_acknacks(&mut self, udp_sender: &UDPSender, ring: &mut IoUring) -> std::io::Result<()> {
        ...
        for (_, writer_proxy) in self.writer_proxies.iter_mut().filter(|(_, p)| p.no_changes_received()) {
          ...
          let bytes = acknack.write_to_vec_with_ctx(Endianness::LittleEndian).unwrap();
    ...
    fn handle_acknack(&mut self, ack_submsg: AckSubmessage, reader_guid_prefix: GuidPrefix) {
                ...
                if let Some(reader_proxy) = self.reader_proxies.get_mut(&reader_guid) {
                    ...
                    reader_proxy.handle_ack_nack(&ack_submsg, last_seq);
                      ...
                      if reader_proxy.all_acked_before > last_seq {
                        ...
                        */
                          todo!("setup repair data for builtins")
                      }
                      //TODO: both of these are in rpts::rtps_reader_proxy
    ...
    fn handle_writer_timed_event(&mut self, ev: &user_data::WriteTimerVariant, udp_sender: &UDPSender, ring: &mut IoUring) {
    ...
    fn remove_all_acked_changes_but_keep_depth(&mut self, depth: Option<usize>, resource_limit: usize) {
        use std::cmp::max;
        let first_keeper = {
    ...
    fn handle_heartbeat_tick(&mut self, manual_assertion: bool, udp_sender: &UDPSender, ring: &mut IoUring) {
        ...
        if self.reader_proxies.values().all(|rp| last_change < rp.all_acked_before) {
...
impl BuiltinParticipantLost {
    pub fn inner<'a, 'b, D: Keyed>(&'b mut self, cache: &'a mut Cache<D, timer_state::Init>) -> BuiltinParticipantLostInner<'a, 'b, D> {
...
impl Caches<timer_state::Uninit> {
    ...
    fn register(self, ring: &mut IoUring, domain_id: u16) -> std::io::Result<Caches<timer_state::Init>> {
        let Caches {
             publications,
             topics,
             participants,
...
impl Caches<timer_state::Init> {
    ...
    pub(crate) fn send_preemptive_acknacks(&mut self, udp_sender: &UDPSender, ring: &mut IoUring) -> std::io::Result<()> {
        self.publications.send_preemptive_acknacks(udp_sender, ring)?;
        self.topics.send_preemptive_acknacks(udp_sender, ring)?;
        self.participants.send_preemptive_acknacks(udp_sender, ring)?;
        self.subscriptions.send_preemptive_acknacks(udp_sender, ring)?;
        self.participant_messages.send_preemptive_acknacks(udp_sender, ring)?;
        Ok(())
    }
    ...
    pub(crate) fn handle_timed_event(&mut self, ev: user_data::Timer, udp_sender: &UDPSender, ring: &mut IoUring) -> Result<(), user_data::Timer> {
        use user_data::Timer;
        Ok(match &ev {
            Timer::Write(entity_id, kind) => {
                match *entity_id {
                    EntityId::SEDP_BUILTIN_PUBLICATIONS_WRITER => self.publications.handle_writer_timed_event(kind, udp_sender, ring),
                    EntityId::SEDP_BUILTIN_TOPIC_WRITER => self.topics.handle_writer_timed_event(kind, udp_sender, ring),
                    EntityId::SPDP_BUILTIN_PARTICIPANT_WRITER => self.participants.handle_writer_timed_event(kind, udp_sender, ring),
                    EntityId::SEDP_BUILTIN_SUBSCRIPTIONS_WRITER => self.subscriptions.handle_writer_timed_event(kind, udp_sender, ring),
                    EntityId::P2P_BUILTIN_PARTICIPANT_MESSAGE_WRITER => self.participant_messages.handle_writer_timed_event(kind, udp_sender, ring),
                    _ => return Err(ev),
                }
            }
            Timer::Read(entity_id, kind) => {
                match *entity_id {
                    EntityId::SEDP_BUILTIN_PUBLICATIONS_READER => self.publications.handle_reader_timed_event(kind),
                    EntityId::SEDP_BUILTIN_TOPIC_READER => self.topics.handle_reader_timed_event(kind),
                    EntityId::SPDP_BUILTIN_PARTICIPANT_READER => self.participants.handle_reader_timed_event(kind),
                    EntityId::SEDP_BUILTIN_SUBSCRIPTIONS_READER => self.subscriptions.handle_reader_timed_event(kind),
                    EntityId::P2P_BUILTIN_PARTICIPANT_MESSAGE_READER => self.participant_messages.handle_reader_timed_event(kind),
                    _ => return Err(ev),
                }
            }
...
impl Discovery2<timer_state::Init> {
    pub fn handle_writer_msg<'a, 'b>(&'a mut self, submsg: WriterSubmessage, mr_state: &MessageReceiverState, discovery_db: &'b mut DiscoveryDB, udp_sender: &UDPSender, ring: &mut IoUring) -> Result<Option<DiscoveredKind>, WriterSubmessage> {
        use crate::messages::submessages::submessage::HasEntityIds;

        let sender_id = submsg.sender_entity_id();

        if !matches!((submsg.receiver_entity_id(), sender_id), 
                     (EntityId::UNKNOWN, EntityId::SEDP_BUILTIN_PUBLICATIONS_WRITER | EntityId::SEDP_BUILTIN_TOPIC_WRITER | EntityId::SPDP_BUILTIN_PARTICIPANT_WRITER | EntityId::SEDP_BUILTIN_SUBSCRIPTIONS_WRITER | EntityId::P2P_BUILTIN_PARTICIPANT_MESSAGE_WRITER)
                     | (EntityId::SEDP_BUILTIN_PUBLICATIONS_READER, EntityId::SEDP_BUILTIN_PUBLICATIONS_WRITER)
                     | (EntityId::SEDP_BUILTIN_TOPIC_READER, EntityId::SEDP_BUILTIN_TOPIC_WRITER)
                     | (EntityId::SEDP_BUILTIN_SUBSCRIPTIONS_READER, EntityId::SEDP_BUILTIN_SUBSCRIPTIONS_WRITER)
                     ) {
    ...
    pub(crate) fn spdp_publish(&mut self, participant_guid: GUID, listeners: &UdpListeners<buf_ring_state::Init>, udp_sender: &UDPSender, ring: &mut IoUring) {
        ...
        self.caches.participants.write::<PlCdrSerializerAdapter<SpdpDiscoveredParticipantData>>(data, udp_sender, ring).unwrap();
    }

    pub fn handle_reader_submsg(&mut self, submsg: ReaderSubmessage, source_guid_prefix: GuidPrefix) -> Result<Option<()>, (ReaderSubmessage, GuidPrefix)> {
        use crate::messages::submessages::submessage::HasEntityIds;
        let sender_id = submsg.sender_entity_id();

        if !matches!((submsg.receiver_entity_id(), sender_id), 
                     (EntityId::SEDP_BUILTIN_PUBLICATIONS_WRITER, EntityId::SEDP_BUILTIN_PUBLICATIONS_READER)
                     | (EntityId::SEDP_BUILTIN_TOPIC_WRITER, EntityId::SEDP_BUILTIN_TOPIC_READER)
                     ...
                     | (EntityId::P2P_BUILTIN_PARTICIPANT_MESSAGE_WRITER, EntityId::P2P_BUILTIN_PARTICIPANT_MESSAGE_READER)
                     ) {
        ...
        match sender_id {
            EntityId::SEDP_BUILTIN_PUBLICATIONS_READER => self.caches.publications.handle_acknack(ack_submsg, source_guid_prefix),
            EntityId::SEDP_BUILTIN_TOPIC_READER => self.caches.topics.handle_acknack(ack_submsg, source_guid_prefix),
            EntityId::SEDP_BUILTIN_SUBSCRIPTIONS_READER => self.caches.subscriptions.handle_acknack(ack_submsg, source_guid_prefix),
            EntityId::SPDP_BUILTIN_PARTICIPANT_READER => self.caches.participants.handle_acknack(ack_submsg, source_guid_prefix),
            EntityId::P2P_BUILTIN_PARTICIPANT_MESSAGE_READER => self.caches.participant_messages.handle_acknack(ack_submsg, source_guid_prefix),
            _ => unreachable!()
        }
        Ok(Some(()))
    }
    ...
    pub fn publish_participant_message(&mut self, discovery_db: &DiscoveryDB, udp_sender: &UDPSender, ring: &mut IoUring) {
        self.timers.participant_messages.reset();

        let min_automatic_lease_duration = discovery_db.get_all_local_topic_writers().filter_map(|p| match p.publication_topic_data.liveliness? {
        ...
        for msg in messages_to_be_sent {
            self.caches.participant_messages.write::<CDRSerializerAdapter<ParticipantMessageData>>(msg, udp_sender, ring);
        }
    }
}

...
impl UdpListeners<buf_ring_state::Uninit> {
    ...
    pub fn register(self, ring: &mut io_uring::IoUring, buf_id: &mut u16, domain_id: u16) -> std::io::Result<UdpListeners<buf_ring_state::Init>> {
        let Self {
            multicast_discovery,
            unicast_discovery,
            multicast_user_traffic,
        ...
        let unicast_discovery = unicast_discovery.register(ring, buf_id, domain_id, UdpDataRecv::UnicastDiscovery)?;

        let multicast_user_traffic = multicast_user_traffic.map(|disc| disc.register(ring, buf_id, domain_id, UdpDataRecv::MulticastUserTraffic)).transpose()?;

        let unicast_user_traffic = unicast_user_traffic.register(ring, buf_id, domain_id, UdpDataRecv::UnicastUserTraffic)?;
